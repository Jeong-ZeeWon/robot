...
function tuneForCategory(category) {
  const plans = {
    ...
    tired: { xp: 6, stats: { kindness: 3, bravery: 1 }, memory: ["mood", "힘든 마음을 말해줘어요"] },
    sad: { xp: 6, stats: { kindness: 3 }, memory: ["mood", "속상한 마음을 나똀어요"] },
    ...
    anxious: { xp: 6, stats: { bravery: 3, kindness: 1 }, memory: ["mood", "불안한 마음을 말해줘어요"] },
    ...
    study: { xp: 5, stats: { curiosity: 2, bravery: 1 }, memory: ["talk", "공부하는 시간에 집중 응원을 받았어요"] },
    exercise: { xp: 6, stats: { bravery: 3, sparkle: 1 }, memory: ["care", "운동하면서 활력 반응을 나눠어요"] },
    music: { xp: 5, stats: { sparkle: 3, curiosity: 1 }, memory: ["play", "음악을 들으며 함께 리듬을 탄어요"] },
    outdoor: { xp: 5, stats: { bravery: 2, curiosity: 2 }, memory: ["play", "외출하며 신선한 기운을 느꼈어요"] },
    grateful: { xp: 7, stats: { kindness: 4, sparkle: 2 }, memory: ["talk", "고마운 마음을 나눠줘서 마음등이 켜졌어요"] },
    work: { xp: 4, stats: { bravery: 2, kindness: 1 }, memory: ["talk", "일하는 시간에 조용히 함께했어요"] },
  };
...
function render(updateFaceFromMood = true) {
  ...
  el.memoryLine.textContent = `전 ${state.visits || 0}번 만났고, ${state.totalTalks || 0}번 이야기했어요. ${memoryContextLine()} ${cooldownLine}`;
  ...
  if (state.lastVisit) {
    ...
    el.lastSeen.textContent = "첫 만났어요";
  }
}

function classify(text) {
  if (includesAny(text, ["안녕", "하이", "hello", "ㅇㅗ"])) return "greeting";
  if (includesAny(text, ["힘들", "피공", "지쳣", "아파", "고단", "녹초"])) return "tired";
  ...
  if (includesAny(text, ["공부", "집중", "과제", "시험", "독서", "학습"])) return "study";
  if (includesAny(text, ["운동", "달리", "헬스", "걷기", "스트레칭"])) return "exercise";
  if (includesAny(text, ["음악", "노래", "멜로디", "리듬"])) return "music";
  if (includesAny(text, ["외출", "산책", "나가", "바깥", "걷어"])) return "outdoor";
  if (includesAny(text, ["고마워", "감사", "고맙", "덕분"])) return "grateful";
  if (includesAny(text, ["일해", "업무", "회의", "야근", "출근", "퇴근"])) return "work";
  if (includesAny(text, ["사랑", "좋아", "귀여", "최고", "잘했"])) return "praise";
  if (includesAny(text, ["잘자", "자자", "졸려", "잌", "자러"])) return "sleep";
  if (includesAny(text, ["배고", "밥", "간식", "먹"])) return "hungry";
  if (includesAny(text, ["기도", "교회", "말씀", "예배", "찬양", "아멘", "할레루야"])) return "faith";
  if (includesAny(text, ["오늘 뽐", "뽐하지", "할 일"])) return "status";
  if (includesAny(text, ["레벨", "상태", "친밀", "몇 번", "기억", "게이지"])) return "status";
  if (includesAny(text, ["이름", "누구", "너는", "소개"])) return "intro";
  if (includesAny(text, ["깜짝", "놓라", "어?"])) return "surprise";
  return "unknown";
}
...
function handlePet() {
  ...
  if (tapCount === 2) {
    completeMission("pet");
    tunePersonality({ sparkle: 2, kindness: 1 });
    gainGrowth(5);
    rememberMoment("care", "더블탭 쓰담으로 시오니가 반짝 반응했어요");
    rememberCare("pet");
    respond("pet", { face: "shy", motion: "squeeze", delta: { mood: 4, affection: 3, energy: -2, loneliness: -5 }, topic: "더블탭", hint: "두 번 빠르게 눈러주시면 시오니가 더 큰 반짝 반응을 보여요." });
    return;
  }
  ...
  rememberCare("pet");
  respond("pet", { delta: effect, topic: "쓰다듬기", hint: "쓰다듬기는 시오니의 친밀도와 기분을 조금 올려주고, 혼자 기다리던 외로움 신호를 부드럽게 낙춰줘요." });
}
...
function handleFeed() {
  ...
  respond("fedSuccess", {
    ...
    topic: "간식 주기",
    motion: "charge",
    hint: "간식은 배고픔를 낙춰주지만, 너무 자주 받으면 시오니가 소화할 시간이 필요해져서 잠깐 쿨타임이 생겨요.",
  });
}
...
function handlePlay() {
  ...
  if (state.energy <= 10) {
    respond("lowEnergy", { delta: { mood: 1, loneliness: -2 }, topic: "에너지 부족", hint: "에너지가 아주 낙으면 먼저 잠깐 쉬게 해주시면, 시오니가 다시 안정적으로 움직이고 대답할 수 있어요." });
    return;
  }
  if (state.hunger >= 94) {
    respond("tooHungry", { delta: { affection: 1 }, topic: "배고픔", hint: "배고픔 게이지가 높을 때는 놀이보다 간식을 먼저 챙기주시면, 시오니가 더 밝고 편안하게 반응할 수 있어요." });
    return;
  }
  if (left > 0) {
    respond("playCooldown", { delta: { energy: -1, hunger: 1 }, topic: "놀이 쿨타임", hint: `놀이는 ${cooldownText(left)} 뒤에 다시 더 즐겠게 할 수 있어요. 지금은 시오니가 방금 움직인 몸을 잠깐 쉬게 하는 중이에요.` });
    return;
  }
...
function handleSleep() {
  ...
  if (left > 0) {
    respond("rested", { delta: { energy: 2, hunger: 1, loneliness: -1 }, topic: "쉬기 쿨타임", hint: `깡은 쉬기는 ${cooldownText(left)} 뒤에 가능하지만, 지금도 시오니가 잠깐 숫을 고르며 에너지를 조금 정리했어요.` });
    return;
  }
  ...
  gainGrowth(4);
  rememberMoment("sleep", "쉬는 시간을 챙겼서 에너지를 회복했어요");
  rememberCare("sleep");
  respond("rested", {
...
function bindEvents() {
  ...
  el.resetButton.addEventListener("click", () => {
    ...
    saveState();
    render(true);
    say("초기화가 완료되었어요. 시오니가 이전 기록을 내려놓고, 다시 첫 만난 마음으로 천천히 친해질 준비를 하고 있어요.", "happy", "bounce");
  });
}
